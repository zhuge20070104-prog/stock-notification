...
**每次 run 结束推一张"📊 大盘简报"** — 基于 PLAN2 的 5 场景规则引擎（VIX / SPY / RSP / HYG / DXY），告诉你今天大盘归属哪个场景，建议仓位方向 + 百分比。**纯规则、不过 LLM、零成本**。同时单股的 LLM prompt 里也注入了宏观场景作为只读上下文，让单股建议跟大盘方向一致。

**分析师共识**也被注入每只股票的 prompt：yfinance 提供的目标价、评级（buy/hold/sell）、分析师数量、Forward EPS — 让 LLM 综合考虑技术面 + 华尔街共识。
...
60 个测试覆盖 macro 规则引擎、advisor prompt 构造、fetcher 分析师字段解析、notifier 简报渲染。conftest 把 yfinance/boto3 stub 掉，无 AWS / 无网络也能跑过。
